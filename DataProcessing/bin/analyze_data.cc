...
void analysis_CLUE(const std::string& in_fname, const std::string& out_fname, const std::string& out_fname2, const std::string& out_fname3, const std::string& in_tname, const SHOWERTYPE& st, const float W0, const float dpos, const bool clean_hits) {
  ...
  /*////////////////////////
    Run custom analyzer
  *////////////////////////
  Analyzer ana(in_fname, in_tname, dc, kappa, ecut, st, W0, dpos, clean_hits);
  ana.runCLUE();
...
int main(int argc, char **argv) {
  ...
  const std::string in_tname = std::string(argv[5]);
  const std::string showertype = std::string(argv[6]);
  const float W0 = std::stof(argv[7]);  
  const float dpos = std::stof(argv[8]);
  const bool clean = std::stoi(argv[9]);  
  ...
  if( showertype=="em" )
    ...
    st = SHOWERTYPE::HAD;
  analysis_CLUE(in_fname, out_fname, out_fname_layer_dependent, out_fname_cluster_dependent, in_tname, st, W0, dpos, clean);
  return 0;
}
